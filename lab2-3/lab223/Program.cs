...
namespace VisualConsoleApp
{
    class Program
    {
        static void Main()
        {
            ...
            {
                case 1:
                    string text1 = File.ReadAllText("D:\\Documents\\лабы ТИ\\input.txt").ToUpper();
                    string key1 = File.ReadAllText("D:\\Documents\\лабы ТИ\\key.txt").ToUpper();

                    string encryptedText1 = Encrypt(text1, key1);
                    File.WriteAllText(@"D:\\Documents\\лабы ТИ\\out.txt", encryptedText1);
                    break;
                case 2:
                    string text2 = File.ReadAllText("D:\\Documents\\лабы ТИ\\out.txt").ToUpper();
                    string key2 = File.ReadAllText("D:\\Documents\\лабы ТИ\\key.txt").ToUpper();

                    string decryptedText2 = Decrypt(text2, key2);
                    Console.WriteLine(decryptedText2);
                    break;
                case 3:
                    string text3 = File.ReadAllText("D:\\Documents\\лабы ТИ\\out.txt").ToUpper();
                    string key3 = File.ReadAllText("D:\\Documents\\лабы ТИ\\key.txt").ToUpper();
        ...
        static string FindKey(string encryptedText, int keyLength)
        {
            ...
            for (int i = 0; i < keyLength; i++)
            {
                ...
                .Where((c, index) => char.IsLetter(c) && index % keyLength == i)// Фильтрация символов, оставляя только буквы и символы, расположенные в столбце ключа.
                .ToArray());
                char mostFrequentChar = column// Находим наиболее часто встречающийся символ в столбце ключа.
                .GroupBy(c => c)
                .Where(group => char.IsLetter(group.Key) && group.Any())// Фильтруем группы
                ...
                {
                    int shift = (mostFrequentChar - 'О' + 32) % 32;// Определение смещения символа
